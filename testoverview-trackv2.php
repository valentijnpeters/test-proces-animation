...
function analyticsDatabaseConfig(): array
{
    ...
    $environmentConfig = [
        ...
        'database' => getenv('TESTOVERVIEW_DB_NAME') ?: 'dbname',
        'username' => getenv('TESTOVERVIEW_DB_USER') ?: 'user',
        'password' => getenv('TESTOVERVIEW_DB_PASS') ?: 'pw',
    ];
...
function analyticsDashboardData(PDO $pdo, string $period, bool $includeIpAddresses = false): array
{
    ...
    $externalLinkCase = 'CASE
        WHEN element_key = "field-guide:more-about-me" OR element_href LIKE "%linkedin.com/in/ing-valentijn-p-b696371%" THEN "More about me"
        WHEN element_key = "field-guide:try-next-level" OR element_href LIKE "%breaksoftwaretesting.com%" THEN "Try the next level"
        ELSE COALESCE(NULLIF(element_label, ""), "External link")
    END';
    $externalLinkWhere = '(element_key IN ("field-guide:more-about-me", "field-guide:try-next-level")
        OR element_href LIKE "%linkedin.com/in/ing-valentijn-p-b696371%"
        OR element_href LIKE "%breaksoftwaretesting.com%")';

    $externalTotalsStatement = $pdo->prepare(
        'SELECT
            cta_label,
            COUNT(*) AS click_count,
            COUNT(DISTINCT session_id) AS session_count,
            MAX(server_received_at) AS last_clicked_at
         FROM (
            SELECT
                ' . $externalLinkCase . ' AS cta_label,
                session_id,
                server_received_at
            FROM test_overview_events
            WHERE application_key = :application_key
              AND event_type = "click"
              AND server_received_at >= ' . $periodStart . '
              AND ' . $externalLinkWhere . '
         ) external_clicks
         GROUP BY cta_label
         ORDER BY click_count DESC, cta_label ASC'
    );
    $externalTotalsStatement->execute([':application_key' => $applicationKey]);
    $externalLinkTotals = $externalTotalsStatement->fetchAll();

    $externalIpAddressSelect = $includeIpAddresses ? 's.ip_address' : 'NULL';
    $externalRecentStatement = $pdo->prepare(
        'SELECT
            CASE
                WHEN e.element_key = "field-guide:more-about-me" OR e.element_href LIKE "%linkedin.com/in/ing-valentijn-p-b696371%" THEN "More about me"
                WHEN e.element_key = "field-guide:try-next-level" OR e.element_href LIKE "%breaksoftwaretesting.com%" THEN "Try the next level"
                ELSE COALESCE(NULLIF(e.element_label, ""), "External link")
            END AS cta_label,
            COALESCE(NULLIF(e.element_key, ""), "unknown") AS element_key,
            e.element_href,
            e.view_key,
            e.page_path,
            ...
            s.session_id,
            s.device_type,
            ' . $externalIpAddressSelect . ' AS ip_address
         FROM test_overview_events e
         LEFT JOIN test_overview_sessions s ON s.session_id = e.session_id
         WHERE e.application_key = :application_key
           AND e.event_type = "click"
           AND e.server_received_at >= ' . $periodStart . '
           AND (e.element_key IN ("field-guide:more-about-me", "field-guide:try-next-level")
                OR e.element_href LIKE "%linkedin.com/in/ing-valentijn-p-b696371%"
                OR e.element_href LIKE "%breaksoftwaretesting.com%")
         ORDER BY e.server_received_at DESC
         LIMIT 18'
    );
    $externalRecentStatement->execute([':application_key' => $applicationKey]);
    $externalLinkRecent = $externalRecentStatement->fetchAll();
    ...
    return [
        ...
        'external_link_clicks' => [
            'totals' => array_map(static function (array $row): array {
                return [
                    'cta_label' => (string) $row['cta_label'],
                    'clicks' => (int) $row['click_count'],
                    'sessions' => (int) $row['session_count'],
                    'last_clicked_at' => (string) ($row['last_clicked_at'] ?? ''),
                ];
            }, $externalLinkTotals),
            'recent' => array_map(static function (array $row) use ($includeIpAddresses): array {
                $clickedAt = (string) ($row['client_occurred_at'] ?: $row['server_received_at'] ?: '');
                $click = [
                    'cta_label' => (string) $row['cta_label'],
                    'element_key' => (string) $row['element_key'],
                ...
                if ($includeIpAddresses) {
                    $click['ip_address'] = (string) ($row['ip_address'] ?: 'unknown');
                }
                return $click;
            }, $externalLinkRecent),
        ],
